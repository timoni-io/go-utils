...
type MapFunc[V any, O any] func(x V) O
...
func MapMap[K comparable, V any, O any](val map[K]V, fn MapFunc[V, O]) map[K]O {
...
// Map creates a new map from calling a function for every map element
func MapSlice2Map[K comparable, V any, O any](val []V, fn func(x V) (K, O)) map[K]O {
	newValues := map[K]O{}
	for _, v := range val {
		k, o := fn(v)
		newValues[k] = o
...
func Until[V any](val []V, fn func(x V) bool) (until []V, rest []V) {
	for i, v := range val {
		if fn(v) {
			until = val[:i]
			rest = val[i:]
